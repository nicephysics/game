...
aim.aim = function(position, velocity, targetPosition, targetVelocity, targetTime, gravity) {
  ...
  if (s < 0 || d === 0) return console.log("Launch is not possible!", s, d)
  var t1 = (q + sr(s)) / d,
      t2 = (q - sr(s)) / d,
      ti
  if (t1 >= tmin && t1 <= tmax && t2 >= tmin && t2 <= tmax) {
    ...
    return console.log("Launch will fail!")
  }
...
console.log(0.5235987755982987 === aim.aim(Vector.create(0, 0), 120, Vector.create(1000, 30), Vector.mult(Vector.create(-Math.cos(Math.PI/6), Math.sin(Math.PI/6)), 100), 5, 9.81))
