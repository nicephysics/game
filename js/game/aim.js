...
aim.aim = function(position, velocity, targetPosition, targetVelocity, targetTime, gravity) {
  ...
  var pi = Math.PI,
      ...
      zzz = s(sin)*sv+2*g*y
  console.log(v, a)
  if (zzz < 0) return console.err("Target projectile never reaches the ground!", zzz)
  var tz = sin*v/g+sr(zzz)/g-t
  if (tz < 0) return console.err("Target projectile already reached destination", -tz, "seconds before launch.")
  var tmin = max(0,(x-cos*t*v)/(cos*v+u)),
      tmax = min(tz,(x-cos*t*v)/(cos*v))
  if (tmin > tmax) return console.err("No suitable launch time!", tmin, tmax)
  ...
  if (s < 0 || d === 0) return console.err("Launch is not possible!", s, d)
  var t1 = (q + sr(s)) / d,
      t2 = (q - sr(s)) / d,
      ti
  if (t1 >= tmin && t1 <= tmax && t2 >= tmin && t2 <= tmax) {
    ...
    return console.err("Launch will fail!")
  }
  var c = (x-cos*v*(t+ti))/(ti*u)
  if (c <= 0.0 || c >= 1.0) return console.err("Incorrect launch angle:", c)
